...
本文转自http://yuangang.cnblogs.com并加以整理。 今天我们来建立几个表，考虑到他们之间的关系，并根据数据库生成EF实体模型。
...
| 19   | GRADUATIONSCHOOL | nvarchar | 200  | 0      |      |      |      | 是     | 毕业院校                                         |
...
当执行完脚本无误后，我们要根据现有的数据库更新EF实体模型。具体操作是在edmx空白区域右键→从数据库更新模型→选择所有表后点击完成
